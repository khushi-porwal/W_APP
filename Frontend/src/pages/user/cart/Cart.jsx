import { ArrowLeft, Minus, Plus, ShoppingBag, Trash2, Truck, Tag, Check, X } from "lucide-react";
...
function Cart() {
    ...
    const handleApplyCoupon = async (e) => {
        ...
        try {
            ...
            const res = await applyCoupon({ code: couponCode.trim().toUpperCase(), totalAmount });
            if (res?.success) {
            ...
            <div className="grid items-start gap-8 lg:grid-cols-12">
                ...
                <div className="lg:col-span-4 space-y-6">
                        ...
                        {appliedCoupon ? (
                            <div className="flex items-center justify-between bg-emerald-50 border border-emerald-200 p-3 rounded-xl">
                                <div>
                                    ...
                                    <p className="text-[10px] text-emerald-600">Saved {formatPrice(couponDiscount)}</p>
                                </div>
                                <button onClick={removeAppliedCoupon} className="text-emerald-700 hover:text-rose-600 p-1">
                                    <X className="w-4 h-4" />
                                </button>
                            </div>
                        ) : (
                            <form onSubmit={handleApplyCoupon} className="flex gap-2">
                                <input
                                    ...
                                    className="flex-1 uppercase text-xs font-bold pl-3 pr-2 py-2.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500"
                                />
                                <button
                                    type="submit"
                                    disabled={applyingCoupon || !couponCode.trim()}
                                    className="bg-indigo-600 hover:bg-indigo-700 text-white font-bold text-xs px-4 py-2.5 rounded-xl transition disabled:opacity-50"
                                >
                                    {applyingCoupon ? "Verifying..." : "Apply"}
                                </button>
                            </form>
                        )}
                        <p className="text-[10px] text-slate-400">Try using WELCOME10 or FLAT500</p>
                    </div>
                    ...
                    <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-xs space-y-4">
                        ...
                        <button
                            onClick={() => navigate("/checkout", { state: { couponCode: appliedCoupon?.code, discountAmount: couponDiscount } })}
