...
function Dashboard() {
    ...
    // Socket.IO Real-time New Order Listener for Admin
    useEffect(() => {
        if (!socket) return;

        const handleNewOrder = (data) => {
            console.log("[Socket.IO Admin] New Order received:", data);
            const formattedId = data.orderId ? data.orderId.slice(-6).toUpperCase() : "";
            
            toast.success(
                `🔔 New Order Received!\nOrder #${formattedId} | Total: ₹${data.totalAmount?.toLocaleString() || 0}`,
                {
                    duration: 6000,
            ...
            // Dynamically update dashboard metrics without reload
            setStats((prev) => ({
                ...prev,
                totalOrders: (prev.totalOrders || 0) + 1,
                totalRevenue: (prev.totalRevenue || 0) + (data.totalAmount || 0)
            }));
        };
