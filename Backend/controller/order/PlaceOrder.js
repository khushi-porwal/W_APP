...
const { emitNewOrder } = require("../../sockets/orderSocket");
const { emitUserNotification } = require("../../sockets/notificationSocket");
const { emitStockUpdate } = require("../../sockets/stockSocket");
...
if (
  ...
  try {
    ...
    // 1. Emit new order alert to admins
    emitNewOrder({
    ...
    // 2. Emit confirmation notification to customer
    emitUserNotification(userId, {
      type: "ORDER_STATUS",
      orderId: order._id.toString(),
      message: `Your order #${formattedOrderId} has been placed successfully!`,
      createdAt: order.createdAt || new Date()
    });

    // 3. Emit real-time stock updates to all connected customers
    for (const item of cartItems) {
      if (item.product && item.product._id) {
        emitStockUpdate(item.product._id.toString(), item.product.stock);
      }
    }
  } catch (socketError) {
    console.error("Failed to emit order placement socket events:", socketError.message);
  }
