...
const updateOrderStatus = asyncHandler(async (req, res) => {
  ...
  try {
    ...
    // Send status update event to specific customer room
    emitOrderStatusUpdate(order._id.toString(), order.user.toString(), order.status, order.updatedAt);
    
    // Send customer notification
    emitUserNotification(order.user.toString(), {
      type: "ORDER_STATUS",
      orderId: order._id.toString(),
      message: `Your order #${formattedOrderId} status has been updated to "${order.status}".`,
      createdAt: new Date()
    });
  } catch (socketError) {
    console.error("Failed to emit order status socket update:", socketError.message);
  }
